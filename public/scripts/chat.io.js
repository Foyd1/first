(function($){
	...
	function bindSocketEvents(){
		...
		socket.on('roomclients', function(data){	
			...
			insertMessage(serverDisplayName, 'Добро пожаловать в комнату : `' + data.room + '`...!', true, false, true);
	...
	function addRoom(name, announce){
		...
		if($('.chat-rooms ul li[data-roomId="' + name + '"]').length == 0){
			...
			if(announce){
				insertMessage(serverDisplayName, 'Комната `' + name + '` создана...', true, false, true);
			}
		}
	}
	...
	function removeRoom(name, announce){
		...
		if(announce){
			insertMessage(serverDisplayName, 'Комната `' + name + '` удалена...', true, false, true);
		}
	}
	...
	function addClient(client, announce, isMe){
		...
		if(announce){
			insertMessage(serverDisplayName, client.nickname + ' зашёл в комнату...', true, false, true);
		}
	...
	function removeClient(client, announce){
		...
		if(announce){
			insertMessage(serverDisplayName, client.nickname + ' покинул комнату...', true, false, true);
		}
	}
	function createRoom(){
